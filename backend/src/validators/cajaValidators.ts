...
export const cierreCajaSchema = z.object({
  caja_id: z.number({ required_error: "El ID de caja es requerido" }),
  usuario_id: z.number({ required_error: "El ID de usuario es requerido" }),
  monto_cierre: z.number({ required_error: "El monto de cierre es requerido" })
    .nonnegative("El monto no puede ser negativo")
    .refine((val) => val.toString().match(/^\d+(\.\d{1,2})?$/), "El monto debe tener máximo 2 decimales"),
});
