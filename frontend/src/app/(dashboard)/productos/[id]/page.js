...
export default function EditarProductoPage() {
    ...
    const fetchData = async () => {
      try {
        const [productoRes, categoriasRes] = await Promise.all([
          productosService.getById(params.id),
        ...
        setFormData({
          ...
          precio_blister: producto.precio_blister || 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await productosService.update(params.id, buildPayload());
      Swal.fire({
        title: 'Éxito',
      ...
      setErrors(e);
      showGlobalAlertFromError(e, 'Error al actualizar producto');
    } finally {
      setLoading(false);
    }
  };

  if (fetching) return <LoadingState type="default" />;
            ...
            <div className="border-t border-zinc-800/50 pt-6 flex gap-3">
              <Button type="submit" disabled={loading}>
                {loading ? (
                  ...
                  <>
                    <Save className="w-4 h-4" /> Actualizar Producto
                  </>
                )}
              </Button>
