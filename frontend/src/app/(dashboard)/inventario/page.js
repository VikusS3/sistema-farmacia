...
export default function InventarioPage() {
  ...
  const filtered = filterTipo
    ? movements.filter((m) => m.movimiento === filterTipo)
    : movements;

  const resumen = {
    entradas: movements
      .filter((m) => ["compra", "devolucion"].includes(m.movimiento))
      .reduce((s, m) => s + Math.abs(m.stock_nuevo - m.stock_anterior), 0),
    salidas: movements
      .filter((m) => ["venta", "vencido"].includes(m.movimiento))
      .reduce((s, m) => s + Math.abs(m.stock_anterior - m.stock_nuevo), 0),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `Stock de "${producto?.nombre}" actualizado a ${formData.cantidad}`,
        timer: 2000,
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: e.response?.data?.error || "Error al registrar ajuste",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8 animate-fade-in">
      <PageHeader
        title="Inventario"
        description={movements.length > 0 ? `${movements.length} movimientos registrados` : "Sin movimientos"}
        actions={
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <Card>
          <CardContent className="flex items-center justify-between py-5">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Entradas</p>
              <p className="text-2xl font-black text-emerald-400">{resumen.entradas}</p>
            </div>
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center border bg-emerald-500/10 border-emerald-500/20">
        ...
        <Card>
          <CardContent className="flex items-center justify-between py-5">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Salidas</p>
              <p className="text-2xl font-black text-red-400">{resumen.salidas}</p>
            </div>
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center border bg-red-500/10 border-red-500/20">
        ...
        <Card>
          <CardContent className="flex items-center justify-between py-5">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Ajustes</p>
              <p className="text-2xl font-black text-amber-400">{resumen.ajustes}</p>
            </div>
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center border bg-amber-500/10 border-amber-500/20">
              ...
              filtered.map((m) => {
                ...
                return (
                  <TableRow key={m.id}>
                    ...
                    <TableCell className={`text-right font-medium ${diff > 0 ? "text-emerald-400" : diff < 0 ? "text-red-400" : "text-zinc-500"}`}>
                      {diff > 0 ? `+${diff}` : diff}
                    </TableCell>
